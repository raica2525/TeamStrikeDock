...
// プレイヤーの初期位置、向き(仮)
#define PLAYER_START_POS1 (D3DXVECTOR3(-500.0f, 0.0f, 0.0f))     // いずれ、ステージの大きさ参照の変数になる
#define PLAYER_START_POS2 (D3DXVECTOR3(500.0f, 0.0f, 0.0f))     // いずれ、ステージの大きさ参照の変数になる
...
#define PLAYER_CONTROL_MOVE_SQUAT 0.96f                     // しゃがみ中の移動量制御
...
#define PLAYER_SQUAT_CUT_COLLISION_SIZE 0.5f                // しゃがみが当たり判定を削る割合
...
// プレイヤーの防御周り
#define PLAYER_LUCKY_GUARD_MAX 10000                        // ラッキーガードの最大値
...
#define PLAYER_COLLISION_SIZE D3DXVECTOR3(100.0f, 300.0f, 100.0f)       // 当たり判定の大きさ（仮）
...
// その他
#define ATTACK_SWING_CHARGE_MAX_FRAME 35            // チャージ最大フレーム数
...
public:
    ...
    void Draw(void);                                                                    // 描画処理
    static CPlayer *Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot, int playable, AI_LEVEL AIlevel = AI_LEVEL_NONE);   // 生成処理

    void Movement(float fSpeed);                                                        // 行動
    void MoveMotion(void);                                                              // 移動モーションの管理
    void CollisionBall(D3DXVECTOR3 playerPos);                                          // ボールに当たったかどうか
    void TakeDamage(float fDamage, int nWho, D3DXVECTOR3 damageOldPos, bool bUseLuckyGuard = false);   // ダメージをくらう
